...
{
    /// <summary>
    /// Interaction logic for ucExameClinico.xaml
    /// </summary>
    public partial class ucExameFisico : UserControlBase
    {
        ...
        private void InsereTabs()
        {
            ...
            foreach (var tab in (this.DataContext as vmSumarioAvaliacaoMedicaRNExameClinico).Tabs)
            {
        ...
        {
            (this.DataContext as vmSumarioAvaliacaoMedicaRNExameClinico).ChamaSalvar();
            if (e.Item.IsNotNull())
            {
                (this.DataContext as vmSumarioAvaliacaoMedicaRNExameClinico).TipoTabSelecionada = ((e.Item as DocumentPanel).ToolTip as vmSumarioAvaliacaoMedicaRNExameClinico.TabsExameClinico?);
        ...
        public void SetaAba()
        {
            dockLayoutManagerN1.Activate(documentGroupN1.Items.Where(x => (vmSumarioAvaliacaoMedicaRNExameClinico.TabsExameClinico)x.ToolTip == (this.DataContext as vmSumarioAvaliacaoMedicaRNExameClinico).TipoTabSelecionada).Single());
        }
    }
}
