...
{
    /// <summary>
    /// Interaction logic for ucObstetrico.xaml
    /// </summary>
    public partial class ucObstetrico : UserControlBase
    {
        ...
        private void InsereTabs()
        {
            ...
            foreach (var tab in (this.DataContext as vmSumarioObstetrico).Tabs)
            {
        ...
        {
            (this.DataContext as vmSumarioObstetrico).ChamaSalvar();
            if (e.Item.IsNotNull())
            {
                (this.DataContext as vmSumarioObstetrico).TipoTabSelecionada = ((e.Item as DocumentPanel).ToolTip as vmSumarioObstetrico.TabsSumarioObstetrico?);
        ...
        public void SetaAba()
        {
            dockLayoutManagerN1.Activate(documentGroupN1.Items.Where(x => (vmSumarioObstetrico.TabsSumarioObstetrico)x.ToolTip == (this.DataContext as vmSumarioObstetrico).TipoTabSelecionada).Single());
        }
    }
}
