...
{
    public class vmPesquisaPacientes : ViewModelBase
    {
        ...
        private void ListaPacientes()
        {
            ...
            if (!Prontuario.IsEmptyOrWhiteSpace())
                rep.OndeCodigoIgual(Prontuario.ToInt());
            ...
            this._listapacienteCollection = new ObservableCollection<wrpPaciente>((from x in rep.List()
                                             select new wrpPaciente(x)).ToList());
